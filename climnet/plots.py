...
def plot_entropy(
    ...
    ax1_2.errorbar(
        x_data, mean_num_groups, yerr=std_num_groups, color="tab:green", label="Groups"
    )
    ax1_2.fill_between(
        x_data,
        mean_num_groups - std_num_groups,
        mean_num_groups + std_num_groups,
        color="tab:green",
        alpha=0.3,
    )
    ...
    ax.legend(loc="upper left", bbox_to_anchor=(
        0.05, 1), bbox_transform=ax.transAxes)
...
def bar_plot_xy(x_arr, data_arr,
                ax=None, label_arr=None, log=False, color_arr=None, **kwargs):
    if ax is None:
    ...
    for idx, arr in enumerate(data_arr):
        ...
        label = label_arr[idx] if label_arr is not None else None
        c = colors[idx] if color_arr is None else color_arr[idx]
        width = 1/len(data_arr)*0.9
        x_pos = np.arange(len(arr)) + width * idx
        ax.bar(
            x_pos,
            arr,
            yerr=None,
    ...
    if len(x_pos) == len(arr):
        ax.set_xticks(np.arange(len(x_arr)) + width/2)
...
def prepare_axis(ax, log=False, **kwargs):
    ...
    ax.grid(False)
    ax.tick_params(
        direction="out", length=SMALL_SIZE / 2, width=1, colors="k", grid_alpha=0.5
    )
...
def prepare_ts_x_axis(ax, dates, **kwargs):
    ...
    sy, ey = tu.get_start_end_date(times=dates)
    ax = prepare_axis(ax, **kwargs)
...
def make_colorbar_discrete(ax, im, fig=None, vmin=None, vmax=None, **kwargs):
    ...
    if sci is not None:
        if sci < 0:
            round_dec = abs(sci)
    ticks = (
        boundaries[::tick_step]
        if round_dec is None
        else np.round_(boundaries[::tick_step], round_dec)
    )

    orientation = kwargs.pop("orientation", "horizontal")
    set_cax = kwargs.pop('set_cax', True)
    ...
    cbar = make_colorbar(ax=ax, im=m, fig=fig, cax=cax, vmin=vmin, vmax=vmax,
                         ticks=ticks, orientation=orientation, sci=sci,
                         **kwargs)

    return cbar


def make_colorbar(ax, im, data=None, fig=None, vmin=None, vmax=None, **kwargs):
...
def set_extent(da, ax, central_longitude=0, **kwargs):

    projection = ccrs.PlateCarree(central_longitude=central_longitude)

    min_ext_lon = np.min(da.coords["lon"])
    max_ext_lon = np.max(da.coords["lon"])
    min_ext_lat = np.min(da.coords["lat"])
    max_ext_lat = np.max(da.coords["lat"])
...
def create_map(
    ...
    **kwargs,
    ...
    ax = set_extent(
        da=da, ax=ax, central_longitude=central_longitude, **kwargs,)
...
def plot_map(
    ...
    **kwargs,
    ...
    if intpol:
        lon_interp = gut.crange(
            dmap.coords["lon"].min(),
            dmap.coords["lon"].max() + ds.grid_step,
            ds.grid_step,
        )
        lat_interp = gut.crange(
            dmap.coords["lat"].min(),
            dmap.coords["lat"].max() + ds.grid_step,
            ds.grid_step,
        )
        ...
        new_values = interp.griddata(
            origin_points, dmap.data, new_points, method="nearest"
        )
    ...
    if plot_type == "scatter":
        ...
        flat_idx_lst = ds.flatten_array(dmap, time=False, check=False)
        ...
        vmin = np.nanmin(dmap)
        vmax = np.nanmax(dmap)
        cmap, norm = discrete_cmap(
            vmin, vmax, colormap=color, num_ticks=levels, shift_ticks=True
        )
        normticks = discrete_norm_ticks(
    ...
    if significant_mask:
        ...
        if intpol:
            new_values_msk = interp.griddata(
                origin_points, mask, new_points, method="nearest"
            )
    ...
    if bar:
        ...
        if plot_type == "contourf":
            make_colorbar_discrete(
                ax, im=im, label=label, vmin=vmin, vmax=vmax, **kwargs
            )

        else:
            cbar = make_colorbar(ax, im=im, data=mesh_values,
                                 label=label, vmin=vmin, vmax=vmax, **kwargs)
...
def plot_wind_field(
    ...
    **kwargs,
    ...
    if stream:
        ...
        wind_unit = kwargs.pop('wind_unit', 2)
        if key_loc:
            ax.quiverkey(
                Q,
                key_loc[0],
                key_loc[1],
                wind_unit,
                f"{wind_unit}"+r"$\frac{m}{s}$",
                labelpos="W",
                coordinates="axes",
            )


def create_multi_map_plot(nrows, ncols, ds=None, projection="EqualEarth",
                          title=None, plt_grid=True, **kwargs):
...
def set_title(title, ax, y_title=1):
    if title is not None:
        y_title = y_title
        ax.set_title(title)
...
def create_multi_plot(nrows, ncols, **kwargs):
...
def text_box(ax, text, pos="upper right"):
...
def make_contour_plot(data, x_coords, y_coords, **kwargs):
