...
class Clim_NetworkX():
    ...
    def __init__(self,
        ...
        self.check_network_dim(self.cnx)
        self.get_nx_info()
    ...
    def create(self, weighted=False):
        ...
        if weighted:
            if self.net.weighted:
                self.weighted = True
                corr = np.where(self.net.adjacency == 1, self.corr, 0)
                # Taking abs to only have positive weights
                cnx = nx.DiGraph(np.abs(corr))
            else:
                raise ValueError(
                    'ERROR! You specified weighted but the network file is unweighted!')
        else:
            cnx = nx.DiGraph(self.net.adjacency)
    ...
    def post_process_cnx(self, cnx):
        ...
        self.check_network_dim(cnx)
        self.cnx = cnx
        self.set_corr_edges()

        return cnx

    def check_network_dim(self, cnx):
        g_N = cnx.number_of_nodes()
        g_E = cnx.number_of_edges()
    ...
    def get_edgelist_th(self, attr, th,
                        upper=True,
                        edge_list=None,
                        sort=True):
        ...
        if sort:
            el, _ = nwf.sort_el_lon_lat(el=el,
                                        netx=self.cnx)
    ...
    def compute_link_lengths_edges(self, backend='multiprocessing',
                                   num_cpus=mpi.cpu_count()):
        """Attribute to each link its spatial length"""
        print('===== Compute spatial link lenghts of all edges ===========')
        link_length = (
        ...
        if var is None:
            for ne in self.cnx.nodes:
                for u, v, e in self.cnx.edges(ne, data=True):
                    ed_l = e['length']
                    ll.append(ed_l)
        elif var == 'formanCurvature' or var == 'betweenness' or var == 'ricciCurvature':
    ...
    def get_long_range_edges(self, min_length, attr=None, q=None):
        ...
        if attr not in self.get_edge_attributes():
            raise ValueError(
                f"{attr} is not a property of the edge attributes!")
        if attr is not None:
            edge_attr = self.get_edge_attr(attr=attr)
            q_val = np.quantile(edge_attr, q=q)

        edge_list = []
        for u, v, a in self.cnx.edges(data=True):
    ...
    def get_spec_loc(self, lon_range, lat_range,
                     attr=None, th=None):
        ...
        ids, loc_map = self.ds.get_locations_in_range(lon_range=lon_range,
                                                      lat_range=lat_range,
                                                      def_map=def_map
                                                      )
    ...
    def get_node_attr_for_edge_list(self, edge_list, attribute, q=None,
                                    attr_suffix="loc",
                                    normalize=True):
    ...
    def get_edges_nodes_for_region(self, lon_range, lat_range,
                                   attribute=None,
                                   binary=False,
                                   q=None):
        ...
        return el_loc, source_map, target_link_map

    def get_edges_for_loc(self, lon, lat,
                          binary=False,
                          ):
    ...
    def normalize_edge_attr(self, attributes=['formanCurvature']):
        ...
        for attr in attributes:
            ...
            edge_attr_norm = (
                ...
                (np.max(edge_attr_val) - np.min(edge_attr_val)) - 1
            )
            # Add to networkx
    ...
    def set_node_attr(self, edge_attrs=['formanCurvature', 'ricciCurvature'],
                      norm=True,
                      attr_name='norm'):
        ...
        for attr in edge_attrs:
            ...
            for ne in self.cnx.nodes:
                ...
                node_cnt = 0
                for u, v, e in self.cnx.edges(ne, data=True):
                    node_sum += e[new_node_attr]
                    node_cnt += 1
                # Normalized attribute by degree
                if node_cnt > 0 and norm is True:
                    # /self.cnx.degree(ne)
                    self.cnx.nodes[ne][new_node_attr] = node_sum / node_cnt
                else:
                    self.cnx.nodes[ne][new_node_attr] = np.nan
    ...
    def get_node_attr_q(self, edge_attrs=['formanCurvature', 'ricciCurvature'],
                        norm=True,
        ...
        for attr in edge_attrs:
            ...
            for q in q_values:
                ...
                for ne in self.cnx.nodes:
                    ...
                    node_cnt = 0
                    for u, v, e in self.cnx.edges(ne, data=True):
                        if (q < 0.5 and e[attr] <= q_val) | (q >= 0.5 and e[attr] >= q_val):
                            node_sum += e[attr]
                            node_cnt += 1
                    # Normalized attribute by degree
