...
from geoutils.utils.statistic_utils import holm
import geoutils.utils.statistic_utils as sut
import geoutils.utils.time_utils as tu
import climnet.network.network_functions as nwf
import geoutils.utils.general_utils as gut
from geoutils.tsa import iaaft
    ...
    with np.load(fname, allow_pickle=True) as data:
        ...
        # event synchronization network
            ...
            Net = EventSyncClimNet(
                dataset, taumax=data["taumax"][0], weighted=weighted)
            if Net.weighted:
                corr = data["corr"]
...
class BaseClimNet:
    ...
    def check_densityApprox(self, adjacency, idx, nn_points_bw=1):
        ...
        bandwidth = lb.obtain_kde_bandwidth(grid=self.ds.grid_step,
...
class CorrClimNet(BaseClimNet):
    ...
    def create(self, **kwargs):
        ...
        **kwargs: dict
        ...
        self.adjacency = self.get_adjacency_corr(
            self.corr,
            self.pvalue,
    ...
    def get_adjacency_corr(
        ...
        **iaaft_kwargs,
        ...
        if significance_test == "bonf" or significance_test == "dunn":
            ...
            q_corr = np.quantile(
                np.abs(self.corr_distribution), q=confidence, axis=0)
